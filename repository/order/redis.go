...
func (r *RedisRepo) Insert(ctx context.Context, order models.Order) error {
	...
	if err != nil {
		return fmt.Errorf("failed tl encode order: %w", err)
	}
...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (models.Order, error) {
	...
	if err := json.Unmarshal([]byte(value), &order); err != nil {
		return models.Order{}, fmt.Errorf("failed to unmarshal: %w", err)
	}
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	...
	for i, x := range xs {
		...
		if err != nil {
			return FindResult{}, fmt.Errorf("failed to unmarshal: %w", err)
		}
